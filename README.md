...
最终可以导出 **可编辑 PowerPoint 和结构化 SVG**。
...
在典型流程图、科研示意图和 AI 生成架构图上，**不调用 VLM、不依赖 Skill，首轮自动重建通常可提供约 80% 的可编辑起点**。剩余工作主要集中在字体、间距、少量布局，以及当前主动舍弃的连接关系上。
...
对于典型流程图、科研示意图和 AI 生成架构图，自动转换后通常再手工调整约 **5–10 分钟**，即可得到一个接近原图、同时又能继续编辑的版本。
